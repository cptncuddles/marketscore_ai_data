...
json_files = glob.glob("C:\\Users\\17197\\Documents\\Coding Projects\\Market Score Data\\Individual Stock Ticker\\*.json")
...
#This double naming is due to meta data stored in the 0 index of the values for the .json's dict object
            ...
            df1 = pd.DataFrame(data_dump, columns=['time_stamp', 'calls', 'puts'])
...
#Need to use zip for this double iterator so that each name get's paired properly with it's corresponding file.
            for df, name in zip(df_list, names):
                df.to_csv(f"C:\\Users\\17197\\Documents\\Coding Projects\\Market Score Data\\Individual Stock Ticker\\{name}.csv", index=False)
